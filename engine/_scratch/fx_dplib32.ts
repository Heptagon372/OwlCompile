// Uint32 variant of fx_dplib (up to 21 mice bits). Scratch library (not engine code): exact min-size program search by function dedup.
...
export interface Model {
  N: number; s0: number; trans: Record<string, Uint32Array>; sensWall: Uint8Array; sensPit: Uint8Array;
}
...
export function buildModel(map: GameMap): Model {
  ...
  if (miceBit.size > 21) throw new Error('too many mice');
  ...
  const trans: Record<string, Uint32Array> = {};
  for (const a of ACTS) {
...
class Store {
  pool: Uint32Array; count = 0; table: Int32Array; hashes: Uint32Array; mask: number; len: number;
  constructor(len: number, cap = 1024) {
    this.len = Math.max(1, len);
    this.pool = new Uint32Array(cap * this.len); this.hashes = new Uint32Array(cap);
    this.table = new Int32Array(cap * 2); this.mask = cap * 2 - 1;
  }
  static hash(a: Uint32Array, len: number): number { let h = 2166136261; for (let i = 0; i < len; i++) { h ^= a[i]; h = Math.imul(h, 16777619); } return h >>> 0; }
  get(i: number): Uint32Array { return this.pool.subarray(i * this.len, (i + 1) * this.len); }
  private grow() {
    const cap = this.hashes.length * 2;
    const p = new Uint32Array(cap * this.len); p.set(this.pool); this.pool = p;
  ...
  find(a: Uint32Array, h = Store.hash(a, this.len)): number {
    let j = h & this.mask;
    for (;;) {
...
export interface SearchOpts {
  ...
  call?: Uint32Array;     // function of the call leaf (def body), optional
...
export function search(m: Model, o: SearchOpts): SearchResult {
  ...
  const compose = (f: Uint32Array, g: Uint32Array, out: Uint32Array) => {
    for (let s = 0; s < N; s++) {
      const v = f[s]; const to = v & 1023;
      if (to >= GOAL) { out[s] = v; continue; }
      const w = g[to]; const wt = w & 1023;
      out[s] = wt === FAIL ? FAIL : ((wt | ((v | w) & 0x7ffffc00)) >>> 0);
    }
  };
  ...
  { const id = new Uint32Array(N); for (let i = 0; i < N; i++) id[i] = i; lists.add(id); listProv.push({ k: 'empty' }); listHasCall.push(false); Lst[0] = [0]; }
  // With requireCall, functions are deduped separately for call/no-call (a call-free twin must not hide a call version).
  const addBlock = (a: Uint32Array, prov: Prov, hc: boolean, cur: number[]) => {
  ...
  const addList = (a: Uint32Array, prov: Prov, hc: boolean, size: number, cur: number[]) => {
    const i = lists.add(a);
  ...
  const updateProj = (k: number) => {
    for (const p of Object.values(projs)) {
      p.reps[k] = []; const buf = new Uint32Array(Math.max(1, p.idx.length));
  ...
  const leaves: [string, Uint32Array][] = ([['f', trans.f], ['J', trans.J], ['L', trans.L], ['R', trans.R]] as [string, Uint32Array][])
      ...
      for (let i = 1; i <= k - 1 && !found; i++) {
        for (const bi of Blk[i]) {
          ...
          for (let mm = 1; mm <= k - i && !found; mm++) for (const li of Lst[mm]) {
            ...
            if (wt === GOAL && good((wt | ((v | w) & 0x7ffffc00)) >>> 0, blockHasCall[bi] || listHasCall[li])) { found = { size: i + mm, prog: [...prog('block', bi), ...prog('list', li)] }; break; }
          }
          if (found) break;
        }
      }
